...
{
	//check if the key is in already
	BTreeNode *tmp = search(k);
	if(tmp) {
		int i = 0;
		while(k > tmp->keys[i])
			i++;
		tmp->offsets[i].push_back(off);
		//cout << "Key already found. Pushing new offset.";
		return;
	}
